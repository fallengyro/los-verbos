(function () {
  ...
  var I18N = {
    es: {
      ...
      check_also_normal_use: "¿También uso normal (no dativo)?",
      gustar_tab_normal: "Uso normal",
      ...
      import_json_intro: "Pegá o subí un archivo JSON con verbos y/o palabras generado por tu propio LLM a partir del formato de Índice Verbal. Nada se guarda hasta que confirmes la importación.",
    ...
    en: {
      ...
      check_also_normal_use: "Also used in its normal (non-dative) form?",
      gustar_tab_normal: "Normal use",
      ...
      import_json_intro: "Paste or upload a JSON file with verbs and/or words generated by your own LLM using Índice Verbal's format. Nothing is saved until you confirm the import.",
  ...
  var STARTER_WORDS = [
    ...
    { word: "remera", definition: "shirt / t-shirt", part_of_speech: "sustantivo", gender: "femenino" },
  ...
  var selectedId = null;
  var editingId = null;
  // Which reading (normal-subject vs. dative) the detail card shows for a
  // verb that is both gustar_like AND also_normal_use (e.g. "parecer") —
  // see the gustar-mode tabs in selectVerb(). Reset to "normal" whenever a
  // different verb is opened; irrelevant (and ignored) for every other verb.
  var detailGustarTab = "normal";
  ...
  var el = {
    ...
    dGustarTabNormal: document.getElementById("d-gustar-tab-normal"),
    ...
    fAlsoNormalUseWrap: document.getElementById("f-also-normal-use-wrap"),
    fAlsoNormalUse: document.getElementById("f-also-normal-use"),
  ...
  function selectVerb(id) {
    if (id !== selectedId) detailGustarTab = "normal";
    selectedId = id;
    var entry = allVerbs.find(function (v) { return v.id === id; });
    if (!entry) { el.detail.hidden = true; return; }
    var data = entry.data;
    var forms = data.forms || {};
    // A verb can be gustar_like AND also_normal_use (e.g. "parecer") — the
    ...
    // shown at all (also_normal_use false, the common case) effectiveGustar
    ...
    var gustarDualMode = !!(data.gustar_like && data.also_normal_use);
    if (el.dGustarTabs) el.dGustarTabs.hidden = !gustarDualMode;
    if (gustarDualMode) {
      el.dGustarTabNormal.classList.toggle("active", detailGustarTab === "normal");
  ...
  function clearForm() {
    ...
    el.fAlsoNormalUse.checked = false;
    el.fAlsoNormalUseWrap.hidden = true;
    el.fGerundio.value = "";
  ...
  function fillForm(data) {
    ...
    el.fAlsoNormalUse.checked = !!data.also_normal_use;
    el.fAlsoNormalUseWrap.hidden = !data.gustar_like;
    var forms = data.forms || {};
  ...
  function collectFormData() {
    ...
    return {
      ...
      also_normal_use: el.fGustarLike.checked && el.fAlsoNormalUse.checked,
      forms: forms
    };
  }
  ...
  function rowToVerb(row) {
    return {
      ...
      data: {
        ...
        also_normal_use: !!row.also_normal_use,
        forms: row.forms || {}
      }
    };
  }
  ...
  function validateImportVerb(v, idx, errorsOut, warningsOut) {
    ...
    return {
      ...
      also_normal_use: !!v.also_normal_use,
      forms: v.forms
    };
  }
  ...
  el.fGustarLike.addEventListener("change", function () {
    el.fAlsoNormalUseWrap.hidden = !el.fGustarLike.checked;
    if (!el.fGustarLike.checked) el.fAlsoNormalUse.checked = false;
  });
  el.dGustarTabNormal.addEventListener("click", function () {
    if (detailGustarTab === "normal") return;
    detailGustarTab = "normal";
    if (selectedId) selectVerb(selectedId);
  });
